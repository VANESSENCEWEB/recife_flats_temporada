  ...
  const thumbs = images.map((img, i) => {
    ...
    return `
      <button type="button"
        ...
        <img src="${src}" alt="" loading="lazy" onerror="this.onerror=null;this.src='${fallback}'">
      </button>
    `;
  }).join('');
    ...
    <div class="apt-detail__gallery apt-gallery apt-gallery--detail" data-gallery tabindex="0">
      <div class="apt-gallery__stage">
        <img class="apt-gallery__main" data-gallery-main src="${mainSrc}" alt="${first.alt}"
             loading="eager" onerror="this.onerror=null;this.src='${mainFallback}'">
...
class RFApartmentDetail extends HTMLElement {
  connectedCallback() {
    ...
    const priceBlock = apt.priceFrom
      ? `<p class="apt-detail__price">a partir de <strong>${apt.priceFrom}</strong>${apt.priceNote}</p>`
      : `<p class="apt-detail__price apt-detail__price--consult">${apt.priceNote}</p>`;

    this.innerHTML = `
      <article class="apt-detail">
        <header class="apt-detail__hero">
          <div class="container apt-detail__hero-inner">
            <rf-breadcrumbs context="apartment" slug="${apt.slug}"></rf-breadcrumbs>
            ${apt.badge ? `<span class="apt-detail__badge">${apt.badge}</span>` : ''}
            <span class="apt-detail__eyebrow">${apt.building} · ${apt.neighborhood}</span>
            <h1 class="apt-detail__title">${apt.name}</h1>
            <p class="apt-detail__tagline">${apt.tagline}</p>
            <div class="apt-detail__rating" aria-label="Nota ${apt.rating} de 5 no Google">
              ★ ${apt.rating.toFixed(1)}${apt.reviewCount ? ` <span>(${apt.reviewCount} avaliações)</span>` : ''}
            </div>
          </div>
        </header>
        ...
        <div class="container apt-detail__layout">
          ...
          <div class="apt-detail__content">
            ...
            ${n ? `
            <section class="apt-detail__block apt-detail__block--nb">
              <h2>Sobre ${n.name}</h2>
              <p>${n.intro}</p>
